...
class Target(object):
    ...
    def jump_to_app(self):
        """The processor is instructed to jump to its app code"""
        raise NotImplementedError("Forcing jump to app code not supported")
    ...
    def target_version(self):
        """\
        Display the target version string.
        """

        print ":".join("{:02x}".format(c) for c in self.version())

    def jump_to_app_code(self):
        """\
        Force the processor to jump to its app code
        """
...
""")
        ...
        group.add_option("--target-version",
                dest="do_target_version",
                help="display the target version number",
                default=False,
                action='store_true')

        group.add_option("-j", "--jump",
                dest="do_jump",
                help="send jump_to_app command",
                default=False,
                action='store_true')
...
""")
    ...
    def parse_args(self):
        ...
        if self.options.do_upload_by_file:
            ...
            default_action = False
        if self.options.do_target_version:
            self.add_action(self.target_version)
            default_action = False
        if self.options.do_jump:
            self.add_action(self.jump_to_app_code)
            default_action = False
